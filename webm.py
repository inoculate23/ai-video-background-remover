...
def doo(video, mode, progress=gr.Progress()):
    ...
    while cap.isOpened():
        ...
        if writer is None:
            writer = cv2.VideoWriter(str(tmpname) + '.webm', cv2.VideoWriter_fourcc(*'VP09'), cap.get(cv2.CAP_PROP_FPS), img.size)
        ...
        out = remover.process(img, type='none')
        writer.write(cv2.cvtColor(np.array(out), cv2.COLOR_BGR2RGB))       

    cap.release()
    writer.release()
    return str(tmpname) + '.webm'
    


description="May got GPU timeout ( Abort / Error ) for longer than 30 second clips."
examples = [['./241872737.webm'],['./crime.mp4']]
